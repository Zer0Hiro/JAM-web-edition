...
jamWeb/
    ...
    jamDsl/
        dsl/                  Compiler pipeline (lexer, parser, codegen, etc.)
...
The jamDsl compiler is included in the `jamDsl/` directory (auto-detected by the server).
...
```bash
cd jamDsl && python3 -m pytest tests/
```
